"""
This module defines how all upcoming appointments will be retrieved for dependents.
"""
...
def upcoming_appointments(child_id):
    """
        Retrieves all upcoming appointments for a child via an authenticated
            parent's account.
    ...
    try:
        ...
        user = storage.get(ChildProfile, current_user_id)
        if not user:
            return jsonify({'error': 'user not found'}), 404

        child = storage.get(LogInfo, child_id)
        if not child:
