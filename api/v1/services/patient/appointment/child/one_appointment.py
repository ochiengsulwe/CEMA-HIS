"""
This module defines how a single is appointment is retrieved for child only.
"""
...
def one_appointment(appointment_id, child_id):
    """
        Retrieves a single appointment for the associated authenticated user's child.
    ...
    try:
        ...
        if appointment not in child.appointments:
            return jsonify({'error': 'not authorised'}), 403

        if child.loginfo.account_type == 'child':
            name_ = appointment.practitioner.identity
        else:
            return jsonify({'error': 'not authorised'}), 404
